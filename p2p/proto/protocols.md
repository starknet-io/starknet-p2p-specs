...
This allows the outbound peer to process data (e.g calculate hashes) before it downloaded all the
data that the inbound peer sent in the session.
...
the data itself. This partition can be done based on the data in the header.
...
The request message is not prefixed with length. The outbound peer should close its writer end once
it sends the request message (it's the only message it should send in the session). The inbound peer
...
After sending all the data, the inbound peer sends a Fin message.
If an inbound peer sends any additional messages after sending a fin message, they are considered
...
If the inbound peer doesn't have all the data for all the blocks it was requested, it should send
...
In order for the outbound peer to know that the inbound peer is malicious as soon as possible, they
...
transactions for the block, this check is a proof of work. It checks that the inbound peer did some
...
should not validate the amount of state diff parts it got for a block and trust that the inbound
peer won't try to DDoS it (and rely on the [limits](#limits) for DDoS prevention).

Aside from DDoS there's no possible attack that can happen. After the inbound peer sent all the data,
the outbound peer can verify it and see if it corresponds to the `state_diff_commitment` that the consensus signed on.
