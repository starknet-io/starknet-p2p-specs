...
replying peer malicious once it sent more transactions than the sum of transaction amounts in the
headers of the requested blocks.
...
In order to avoid potential DDoS, the querying peer should reject the connection and declare the
replying peer malicious once it sent more receipts than the sum of transaction amounts in the
headers of the requested blocks.
...
Since currently the class is stored inside a single message, we can't use the length to reject a
node DDoSing us with a large class. If we decide to split the class to multiple messages, then each
node should reject peers that sent too many class messages for a single class.
